...
class Worm extends Enemy {
    constructor(x, y) {
        ...
        this.originPosition = createVector(x, y); // usually make rounds around there  
        this.damage = 2;
        this.attackPattern = 0; //0 will always be standoff stance
        this.speed = 4;
        this.width = 250;
        this.height = 350;
        this.hp = 120;
        this.mhp = 120;
        this.grounded = true; //burrowed
        this.jumpSpeed = 25;
        this.currentJumpSpeed = 0;
        this.floorY;
        this.image = worm;
        this.factor = 20; // ??
        this.attackRange = 50; // should actually vary
        this.onRange = 3500; // when to start processing 
        this.inAttackFor = 0; //max time in stance
        this.target;
        this.stamina;
        this.attackCooldown = 0; // after attacking perhaps chains into another, otherwise assume stance 0 
        this.takenDamageMultiplier = 1;
        this.hoverY;
        this.tenacity = 100;
        this.standoff = 1;
    ...
    wander() {
        //move around origin location - 1000 to origin location + 1000
        if (this.position.x <= this.originPosition.x - 1400) {
            this.direction = 1;
        } else if (this.position.x >= this.originPosition.x + 1400) {
            this.direction = -1;
        }
        //make the rounds 
        this.position.add(this.speed * this.direction, 0);
        this.position.y = this.floorY + 300;
    ...
    attack() {
        if (this.attackPattern === 0) { // basic standoff, movement etc 
            ...
            if (Math.abs(this.position.x - this.target.position.x) <= this.attackRange) {
                this.canStillDamage = true;
                this.attackPattern = 1;
                this.inAttackFor = 60;
            }
            ...
            this.jumpSpeed = 14;
            this.position.add(0, -this.jumpSpeed);
            if (this.position.y + this.height / 2 <= this.floorY) {
                this.attackPattern = 2; // just enough to last for the surfacing 
                this.inAttackFor = floor(random(220, 261));
                this.position.y = this.floorY - this.height / 2;
            }

            if (collisionDetected(this.target, this.position, this.width / 2, this.height / 2)) {
                if (this.canStillDamage) {
                    damageAlly(this.target, 5);
                }
            ...
            if (this.inAttackFor % 40 == 0) {
                projectiles.push(new AcidBall(this.position.x, this.position.y - 35, this.target, 70, 70, 3, 7 * this.direction, 55, 7.5));
            }
            ...
            if (this.inAttackFor == 1) {
                this.attackPattern = 3; // dive
                this.inAttackFor = 120;
            }
            ...
            this.jumpSpeed = 8;
            this.position.add(0, this.jumpSpeed);
            if (this.position.y> this.floorY + 300) {
                this.attackPattern = 0;
                this.inAttackFor = 1;
    ...
    process(e) {
        ...
        if (player.position.dist(this.position) < this.onRange) {
            ...
            if (this.target != null && this.attackCooldown === 0) {
                //select attackPattern
                if (this.inAttackFor === 0) {
                    this.originPosition.x = this.target.position.x + random(-600, 600);
                    if (this.target.position.x < this.position.x) {
                        this.direction = -1;
                    } else {
                        this.direction = 1;
                    ...
                    //criteria for different attack patterns
                    if (Math.abs(this.position.x - this.target.position.x) > 120) {
                        this.attackPattern = 0; //standoff which is wander
                        this.inAttackFor = 5;
                    } else if (this.position.x - this.target.position.x <= 120 && this.attackPattern === 0) {
                        this.canStillDamage = true;
                        console.log("saddd")
                        this.attackPattern = 1;
                        this.inAttackFor = 60;
